...
const fontFamilyMap = {
  inter: "var(--font-geist-sans), ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, 'Noto Sans', sans-serif",
  roboto: "var(--font-roboto), Roboto, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Helvetica Neue', Arial, 'Noto Sans', sans-serif",
...
export function useFontSettings() {
  ...
  useEffect(() => {
    ...
    if (savedFontFamily && savedFontFamily !== "inter" || savedFontSize && savedFontSize !== "medium") {
  ...
  const applyFontSettings = (settings: FontSettings) => {
    ...
    html.style.setProperty('font-family', fontFamilyMap[settings.fontFamily], 'important');
    body.style.setProperty('font-family', fontFamilyMap[settings.fontFamily], 'important');
    
    // Apply font size
    html.style.setProperty('font-size', fontSizeMap[settings.fontSize], 'important');
    body.style.setProperty('font-size', fontSizeMap[settings.fontSize], 'important');
    
    // Also set CSS custom properties for other components that might use them
    html.style.setProperty("--font-family-custom", fontFamilyMap[settings.fontFamily]);
    html.style.setProperty("--font-size-custom", fontSizeMap[settings.fontSize]);
    
    // Add a class to body for CSS targeting
    body.className = body.className.replace(/font-\w+/g, '');
    body.classList.add(`font-${settings.fontFamily}`, `size-${settings.fontSize}`);
  };
